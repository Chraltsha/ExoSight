var e = Object.create,
	t = Object.defineProperty,
	n = Object.getOwnPropertyDescriptor,
	r = Object.getOwnPropertyNames,
	i = Object.getPrototypeOf,
	a = Object.prototype.hasOwnProperty,
	o = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), (e = null)), t.exports),
	s = (e, n) => {
		let r = {};
		for (var i in e) t(r, i, { get: e[i], enumerable: !0 });
		return (n || t(r, Symbol.toStringTag, { value: `Module` }), r);
	},
	c = (e, i, o, s) => {
		if ((i && typeof i == `object`) || typeof i == `function`)
			for (var c = r(i), l = 0, u = c.length, d; l < u; l++)
				((d = c[l]),
					!a.call(e, d) &&
						d !== o &&
						t(e, d, {
							get: ((e) => i[e]).bind(null, d),
							enumerable: !(s = n(i, d)) || s.enumerable,
						}));
		return e;
	},
	l = (n, r, o) => (
		(o = n == null ? {} : e(i(n))),
		c(
			r || !n || !n.__esModule || !a.call(n, `default`)
				? t(o, `default`, { value: n, enumerable: !0 })
				: o,
			n,
		)
	),
	u = Array.isArray,
	d = Array.prototype.indexOf,
	f = Array.prototype.includes,
	p = Array.from,
	m = Object.defineProperty,
	h = Object.getOwnPropertyDescriptor,
	g = Object.getOwnPropertyDescriptors,
	_ = Object.prototype,
	v = Array.prototype,
	y = Object.getPrototypeOf,
	b = Object.isExtensible;
function x(e) {
	return typeof e == `function`;
}
var S = () => {};
function ee(e) {
	for (var t = 0; t < e.length; t++) e[t]();
}
function te() {
	var e, t;
	return {
		promise: new Promise((n, r) => {
			((e = n), (t = r));
		}),
		resolve: e,
		reject: t,
	};
}
var C = 1024,
	w = 2048,
	ne = 4096,
	re = 8192,
	ie = 16384,
	ae = 32768,
	oe = 1 << 25,
	se = 65536,
	ce = 1 << 18,
	le = 1 << 19,
	ue = 1 << 20,
	de = 1 << 25,
	fe = 65536,
	pe = 1 << 21,
	me = 1 << 22,
	he = 1 << 23,
	ge = Symbol(`$state`),
	_e = Symbol(`legacy props`),
	ve = Symbol(``),
	ye = Symbol(`attributes`),
	be = Symbol(`class`),
	xe = Symbol(`style`),
	Se = Symbol(`text`),
	Ce = Symbol(`form reset`),
	we = new (class extends Error {
		name = `StaleReactionError`;
		message = 'The reaction that called `getAbortSignal()` was re-run or destroyed';
	})(),
	Te = !!globalThis.document?.contentType && globalThis.document.contentType.includes(`xml`);
function Ee(e) {
	throw Error(`https://svelte.dev/e/experimental_async_required`);
}
function De(e) {
	throw Error(`https://svelte.dev/e/lifecycle_outside_component`);
}
function Oe() {
	throw Error(`https://svelte.dev/e/missing_context`);
}
function ke() {
	throw Error(`https://svelte.dev/e/async_derived_orphan`);
}
function Ae(e, t, n) {
	throw Error(`https://svelte.dev/e/each_key_duplicate`);
}
function je(e) {
	throw Error(`https://svelte.dev/e/effect_in_teardown`);
}
function Me() {
	throw Error(`https://svelte.dev/e/effect_in_unowned_derived`);
}
function Ne(e) {
	throw Error(`https://svelte.dev/e/effect_orphan`);
}
function Pe() {
	throw Error(`https://svelte.dev/e/effect_update_depth_exceeded`);
}
function Fe() {
	throw Error(`https://svelte.dev/e/fork_discarded`);
}
function Ie() {
	throw Error(`https://svelte.dev/e/fork_timing`);
}
function Le() {
	throw Error(`https://svelte.dev/e/get_abort_signal_outside_reaction`);
}
function Re() {
	throw Error(`https://svelte.dev/e/hydration_failed`);
}
function ze(e) {
	throw Error(`https://svelte.dev/e/lifecycle_legacy_only`);
}
function Be(e) {
	throw Error(`https://svelte.dev/e/props_invalid_value`);
}
function Ve() {
	throw Error(`https://svelte.dev/e/state_descriptors_fixed`);
}
function He() {
	throw Error(`https://svelte.dev/e/state_prototype_fixed`);
}
function Ue() {
	throw Error(`https://svelte.dev/e/state_unsafe_mutation`);
}
function We() {
	throw Error(`https://svelte.dev/e/svelte_boundary_reset_onerror`);
}
var Ge = {},
	T = Symbol(`uninitialized`),
	Ke = `http://www.w3.org/1999/xhtml`;
function qe() {
	console.warn(`https://svelte.dev/e/derived_inert`);
}
function Je(e) {
	console.warn(`https://svelte.dev/e/hydratable_missing_but_expected`);
}
function Ye(e) {
	console.warn(`https://svelte.dev/e/hydration_mismatch`);
}
function Xe() {
	console.warn(`https://svelte.dev/e/svelte_boundary_reset_noop`);
}
var E = !1;
function D(e) {
	E = e;
}
var O;
function k(e) {
	if (e === null) throw (Ye(), Ge);
	return (O = e);
}
function Ze() {
	return k(R(O));
}
function Qe(e) {
	if (E) {
		if (R(O) !== null) throw (Ye(), Ge);
		O = e;
	}
}
function $e(e = 1) {
	if (E) {
		for (var t = e, n = O; t--;) n = R(n);
		O = n;
	}
}
function et(e = !0) {
	for (var t = 0, n = O; ;) {
		if (n.nodeType === 8) {
			var r = n.data;
			if (r === `]`) {
				if (t === 0) return n;
				--t;
			} else (r === `[` || r === `[!` || (r[0] === `[` && !isNaN(Number(r.slice(1))))) && (t += 1);
		}
		var i = R(n);
		(e && n.remove(), (n = i));
	}
}
function tt(e) {
	if (!e || e.nodeType !== 8) throw (Ye(), Ge);
	return e.data;
}
function nt(e) {
	return e === this.v;
}
function rt(e, t) {
	return e == e ? e !== t || (typeof e == `object` && !!e) || typeof e == `function` : t == t;
}
function it(e) {
	return !rt(e, this.v);
}
var A = null;
function at(e) {
	A = e;
}
function ot() {
	let e = {};
	return [() => (lt(e) || Oe(), st(e)), (t) => ct(e, t)];
}
function st(e) {
	return mt(`getContext`).get(e);
}
function ct(e, t) {
	return (mt(`setContext`).set(e, t), t);
}
function lt(e) {
	return mt(`hasContext`).has(e);
}
function ut() {
	return mt(`getAllContexts`);
}
function dt(e, t = !1, n) {
	A = { p: A, i: !1, c: null, e: null, s: e, x: null, r: G, l: null };
}
function ft(e) {
	var t = A,
		n = t.e;
	if (n !== null) {
		t.e = null;
		for (var r of n) Kn(r);
	}
	return (e !== void 0 && (t.x = e), (t.i = !0), (A = t.p), e ?? {});
}
function pt() {
	return !0;
}
function mt(e) {
	return (A === null && De(e), (A.c ??= new Map(ht(A) || void 0)));
}
function ht(e) {
	let t = e.p;
	for (; t !== null;) {
		let e = t.c;
		if (e !== null) return e;
		t = t.p;
	}
	return null;
}
var gt = [];
function _t() {
	var e = gt;
	((gt = []), ee(e));
}
function j(e) {
	if (gt.length === 0 && !en) {
		var t = gt;
		queueMicrotask(() => {
			t === gt && _t();
		});
	}
	gt.push(e);
}
function vt() {
	for (; gt.length > 0;) _t();
}
function yt(e) {
	var t = G;
	if (t === null) return ((H.f |= he), e);
	if (!(t.f & 32768) && !(t.f & 4)) throw e;
	bt(e, t);
}
function bt(e, t) {
	if (!(t !== null && t.f & 16384)) {
		for (; t !== null;) {
			if (t.f & 128) {
				if (!(t.f & 32768)) throw e;
				try {
					t.b.error(e);
					return;
				} catch (t) {
					e = t;
				}
			}
			t = t.parent;
		}
		throw e;
	}
}
var xt = ~(w | ne | C);
function M(e, t) {
	e.f = (e.f & xt) | t;
}
function St(e) {
	e.f & 512 || e.deps === null ? M(e, C) : M(e, ne);
}
function Ct(e) {
	if (e !== null) for (let t of e) !(t.f & 2) || !(t.f & 65536) || ((t.f ^= fe), Ct(t.deps));
}
function wt(e, t, n) {
	(e.f & 2048 ? t.add(e) : e.f & 4096 && n.add(e), Ct(e.deps), M(e, C));
}
var Tt = [];
function Et(e, t = S) {
	let n = null,
		r = new Set();
	function i(t) {
		if (rt(e, t) && ((e = t), n)) {
			let t = !Tt.length;
			for (let t of r) (t[1](), Tt.push(t, e));
			if (t) {
				for (let e = 0; e < Tt.length; e += 2) Tt[e][0](Tt[e + 1]);
				Tt.length = 0;
			}
		}
	}
	function a(t) {
		i(t(e));
	}
	function o(o, s = S) {
		let c = [o, s];
		return (
			r.add(c),
			r.size === 1 && (n = t(i, a) || S),
			o(e),
			() => {
				(r.delete(c), r.size === 0 && n && (n(), (n = null)));
			}
		);
	}
	return { set: i, update: a, subscribe: o };
}
var Dt = !1;
function Ot(e) {
	var t = Dt;
	try {
		return ((Dt = !1), [e(), Dt]);
	} finally {
		Dt = t;
	}
}
var kt = !1;
function At() {
	kt ||
		((kt = !0),
		document.addEventListener(
			`reset`,
			(e) => {
				Promise.resolve().then(() => {
					if (!e.defaultPrevented) for (let t of e.target.elements) t[Ce]?.();
				});
			},
			{ capture: !0 },
		));
}
function jt(e) {
	var t = H,
		n = G;
	(W(null), K(null));
	try {
		return e();
	} finally {
		(W(t), K(n));
	}
}
function Mt(e, t, n, r = n) {
	e.addEventListener(t, () => jt(n));
	let i = e[Ce];
	((e[Ce] = i
		? () => {
				(i(), r(!0));
			}
		: () => r(!0)),
		At());
}
function Nt(e) {
	let t = 0,
		n = bn(0),
		r;
	return () => {
		Un() &&
			(Z(n),
			Zn(
				() => (
					t === 0 && (r = Q(() => e(() => Tn(n)))),
					(t += 1),
					() => {
						j(() => {
							(--t, t === 0 && (r?.(), (r = void 0), Tn(n)));
						});
					}
				),
			));
	};
}
var Pt = se | le;
function Ft(e, t, n, r) {
	new It(e, t, n, r);
}
var It = class {
	parent;
	is_pending = !1;
	transform_error;
	#e;
	#t = E ? O : null;
	#n;
	#r;
	#i;
	#a = null;
	#o = null;
	#s = null;
	#c = null;
	#l = 0;
	#u = 0;
	#d = !1;
	#f = new Set();
	#p = new Set();
	#m = null;
	#h = Nt(
		() => (
			(this.#m = bn(this.#l)),
			() => {
				this.#m = null;
			}
		),
	);
	constructor(e, t, n, r) {
		((this.#e = e),
			(this.#n = t),
			(this.#r = (e) => {
				var t = G;
				((t.b = this), (t.f |= 128), n(e));
			}),
			(this.parent = G.b),
			(this.transform_error = r ?? this.parent?.transform_error ?? ((e) => e)),
			(this.#i = $n(() => {
				if (E) {
					let e = this.#t;
					Ze();
					let t = e.data === `[!`;
					if (e.data.startsWith(`[?`)) {
						let t = JSON.parse(e.data.slice(2));
						this.#_(t);
					} else t ? this.#y() : this.#g();
				} else this.#b();
			}, Pt)),
			E && (this.#e = O));
	}
	#g() {
		try {
			this.#a = B(() => this.#r(this.#e));
		} catch (e) {
			this.error(e);
		}
	}
	#_(e) {
		let t = this.#n.failed,
			{ reset: n, invoke_onerror: r } = this.#v(e);
		(j(r),
			t &&
				(this.#s = B(() => {
					t(
						this.#e,
						() => e,
						() => n,
					);
				})));
	}
	#v(e) {
		var t = !1,
			n = !1;
		let r = () => {
			if (t) {
				Xe();
				return;
			}
			((t = !0),
				n && We(),
				this.#s !== null &&
					ar(this.#s, () => {
						this.#s = null;
					}),
				this.#S(() => {
					this.#b();
				}));
		};
		return {
			reset: r,
			invoke_onerror: () => {
				try {
					((n = !0), this.#n.onerror?.(e, r), (n = !1));
				} catch (e) {
					bt(e, this.#i && this.#i.parent);
				}
			},
		};
	}
	#y() {
		let e = this.#n.pending;
		e &&
			((this.is_pending = !0),
			(this.#o = B(() => e(this.#e))),
			j(() => {
				var e = (this.#c = document.createDocumentFragment()),
					t = L();
				(e.append(t),
					(this.#a = this.#S(() => B(() => this.#r(t)))),
					this.#u === 0 &&
						(this.#e.before(e),
						(this.#c = null),
						ar(this.#o, () => {
							this.#o = null;
						}),
						this.#x(N)));
			}));
	}
	#b() {
		try {
			if (
				((this.is_pending = this.has_pending_snippet()),
				(this.#u = 0),
				(this.#l = 0),
				(this.#a = B(() => {
					this.#r(this.#e);
				})),
				this.#u > 0)
			) {
				var e = (this.#c = document.createDocumentFragment());
				lr(this.#a, e);
				let t = this.#n.pending;
				this.#o = B(() => t(this.#e));
			} else this.#x(N);
		} catch (e) {
			this.error(e);
		}
	}
	#x(e) {
		((this.is_pending = !1), e.transfer_effects(this.#f, this.#p));
	}
	defer_effect(e) {
		wt(e, this.#f, this.#p);
	}
	is_rendered() {
		return !this.is_pending && (!this.parent || this.parent.is_rendered());
	}
	has_pending_snippet() {
		return !!this.#n.pending;
	}
	#S(e) {
		var t = G,
			n = H,
			r = A;
		(K(this.#i), W(this.#i), at(this.#i.ctx));
		try {
			return (sn.ensure(), e());
		} catch (e) {
			return (yt(e), null);
		} finally {
			(K(t), W(n), at(r));
		}
	}
	#C(e, t) {
		if (!this.has_pending_snippet()) {
			this.parent && this.parent.#C(e, t);
			return;
		}
		((this.#u += e),
			this.#u === 0 &&
				(this.#x(t),
				this.#o &&
					ar(this.#o, () => {
						this.#o = null;
					}),
				(this.#c &&= (this.#e.before(this.#c), null))));
	}
	update_pending_count(e, t) {
		(this.#C(e, t),
			(this.#l += e),
			!(!this.#m || this.#d) &&
				((this.#d = !0),
				j(() => {
					((this.#d = !1), this.#m && Cn(this.#m, this.#l));
				})));
	}
	get_effect_pending() {
		return (this.#h(), Z(this.#m));
	}
	error(e) {
		if (!this.#n.onerror && !this.#n.failed) throw e;
		N?.is_fork
			? (this.#a && N.skip_effect(this.#a),
				this.#o && N.skip_effect(this.#o),
				this.#s && N.skip_effect(this.#s),
				N.oncommit(() => {
					this.#w(e);
				}))
			: this.#w(e);
	}
	#w(e) {
		((this.#a &&= (V(this.#a), null)),
			(this.#o &&= (V(this.#o), null)),
			(this.#s &&= (V(this.#s), null)),
			E && (k(this.#t), $e(), k(et())));
		let t = this.#n.failed,
			n = (e) => {
				let { reset: n, invoke_onerror: r } = this.#v(e);
				(r(),
					t &&
						(this.#s = this.#S(() => {
							try {
								return B(() => {
									var r = G;
									((r.b = this),
										(r.f |= 128),
										t(
											this.#e,
											() => e,
											() => n,
										));
								});
							} catch (e) {
								return (bt(e, this.#i.parent), null);
							}
						})));
			};
		j(() => {
			var t;
			try {
				t = this.transform_error(e);
			} catch (e) {
				bt(e, this.#i && this.#i.parent);
				return;
			}
			typeof t == `object` && t && typeof t.then == `function`
				? t.then(n, (e) => bt(e, this.#i && this.#i.parent))
				: n(t);
		});
	}
};
function Lt(e, t, n, r) {
	let i = pt() ? Vt : Gt;
	var a = e.filter((e) => !e.settled),
		o = t.map(i);
	if (n.length === 0 && a.length === 0) {
		r(o);
		return;
	}
	var s = G,
		c = Rt(),
		l = a.length === 1 ? a[0].promise : a.length > 1 ? Promise.all(a.map((e) => e.promise)) : null;
	function u(e) {
		if (!(s.f & 16384)) {
			c();
			try {
				r([...o, ...e]);
			} catch (e) {
				bt(e, s);
			}
			zt();
		}
	}
	var d = Bt();
	if (n.length === 0) {
		l.then(() => u([])).finally(d);
		return;
	}
	function f() {
		Promise.all(n.map((e) => Ut(e)))
			.then(u)
			.catch((e) => bt(e, s))
			.finally(d);
	}
	l
		? l.then(() => {
				(c(), f(), zt());
			})
		: f();
}
function Rt() {
	var e = G,
		t = H,
		n = A,
		r = N;
	return function (i = !0) {
		(K(e), W(t), at(n), i && !(e.f & 16384) && (r?.activate(), r?.apply()));
	};
}
function zt(e = !0) {
	(K(null), W(null), at(null), e && N?.deactivate());
}
function Bt() {
	var e = G,
		t = e.b,
		n = N,
		r = !!t?.is_rendered();
	return (
		t?.update_pending_count(1, n),
		n.increment(r, e),
		() => {
			(t?.update_pending_count(-1, n), n.decrement(r, e));
		}
	);
}
function Vt(e) {
	var t = 2 | w;
	return (
		G !== null && (G.f |= le),
		{
			ctx: A,
			deps: null,
			effects: null,
			equals: nt,
			f: t,
			fn: e,
			reactions: null,
			rv: 0,
			v: T,
			wv: 0,
			parent: G,
			ac: null,
		}
	);
}
var Ht = Symbol(`obsolete`);
function Ut(e, t, n) {
	let r = G;
	r === null && ke();
	var i = void 0,
		a = bn(T),
		o = !H,
		s = new Set();
	return (
		Xn(() => {
			var t = G,
				n = te();
			i = n.promise;
			try {
				Promise.resolve(e())
					.then(n.resolve, (e) => {
						e !== we && n.reject(e);
					})
					.finally(zt);
			} catch (e) {
				(n.reject(e), zt());
			}
			var c = N;
			if (o) {
				if (t.f & 32768) var l = Bt();
				if (r.b?.is_rendered()) c.async_deriveds.get(t)?.reject(Ht);
				else for (let e of s.values()) e.reject(Ht);
				(s.add(n), c.async_deriveds.set(t, n));
			}
			let u = (e, t = void 0) => {
				(l?.(),
					s.delete(n),
					t !== Ht &&
						(c.activate(),
						t ? ((a.f |= he), Cn(a, t)) : (a.f & 8388608 && (a.f ^= he), Cn(a, e)),
						c.deactivate()));
			};
			n.promise.then(u, (e) => u(null, e || `unknown`));
		}),
		Wn(() => {
			for (let e of s) e.reject(Ht);
		}),
		new Promise((e) => {
			function t(n) {
				function r() {
					n === i ? e(a) : t(i);
				}
				n.then(r, r);
			}
			t(i);
		})
	);
}
function Wt(e) {
	let t = Vt(e);
	return (mr(t), t);
}
function Gt(e) {
	let t = Vt(e);
	return ((t.equals = it), t);
}
function Kt(e) {
	var t = e.effects;
	if (t !== null) {
		e.effects = null;
		for (var n = 0; n < t.length; n += 1) V(t[n]);
	}
}
function qt(e) {
	var t,
		n = G,
		r = e.parent;
	if (!fr && r !== null && e.v !== T && r.f & 24576) return (qe(), e.v);
	K(r);
	try {
		((e.f &= ~fe), Kt(e), (t = Cr(e)));
	} finally {
		K(n);
	}
	return t;
}
function Jt(e) {
	var t = qt(e);
	if (
		!e.equals(t) &&
		((e.wv = br()),
		(!N?.is_fork || e.deps === null) &&
			(N === null ? (e.v = t) : (N.capture(e, t, !0), Qt?.capture(e, t, !0)), e.deps === null))
	) {
		M(e, C);
		return;
	}
	fr || (P === null ? St(e) : (Un() || N?.is_fork) && P.set(e, t));
}
function Yt(e) {
	if (e.effects !== null)
		for (let t of e.effects)
			(t.teardown || t.ac) &&
				(t.teardown?.(),
				t.ac !== null &&
					jt(() => {
						(t.ac.abort(we), (t.ac = null));
					}),
				t.fn !== null && (t.teardown = S),
				Tr(t, 0),
				tr(t));
}
function Xt(e) {
	if (e.effects !== null) for (let t of e.effects) t.teardown && t.fn !== null && Er(t);
}
var Zt = null,
	N = null,
	Qt = null,
	P = null,
	$t = null,
	en = !1,
	tn = !1,
	nn = null,
	rn = null,
	an = 0,
	on = 1,
	sn = class e {
		id = on++;
		#e = !1;
		linked = !0;
		#t = null;
		#n = null;
		async_deriveds = new Map();
		current = new Map();
		previous = new Map();
		#r = new Set();
		#i = new Set();
		#a = 0;
		#o = new Map();
		#s = null;
		#c = [];
		#l = [];
		#u = new Set();
		#d = new Set();
		#f = new Map();
		#p = new Set();
		is_fork = !1;
		#m = !1;
		constructor() {
			(Zt === null ? (Zt = this) : ((Zt.#n = this), (this.#t = Zt)), (Zt = this));
		}
		#h() {
			if (this.is_fork) return !0;
			for (let n of this.#o.keys()) {
				for (var e = n, t = !1; e.parent !== null;) {
					if (this.#f.has(e)) {
						t = !0;
						break;
					}
					e = e.parent;
				}
				if (!t) return !0;
			}
			return !1;
		}
		skip_effect(e) {
			(this.#f.has(e) || this.#f.set(e, { d: [], m: [] }), this.#p.delete(e));
		}
		unskip_effect(e, t = (e) => this.schedule(e)) {
			var n = this.#f.get(e);
			if (n) {
				this.#f.delete(e);
				for (var r of n.d) (M(r, w), t(r));
				for (r of n.m) (M(r, ne), t(r));
			}
			this.#p.add(e);
		}
		#g() {
			((this.#e = !0), an++ > 1e3 && (this.#x(), ln()));
			for (let e of this.#u) (this.#d.delete(e), M(e, w), this.schedule(e));
			for (let e of this.#d) (M(e, ne), this.schedule(e));
			let t = this.#c;
			((this.#c = []), this.apply());
			var n = (nn = []),
				r = [],
				i = (rn = []);
			for (let e of t)
				try {
					this.#_(e, n, r);
				} catch (t) {
					throw (mn(e), this.#h() || this.discard(), t);
				}
			if (((N = null), i.length > 0)) {
				var a = e.ensure();
				for (let e of i) a.schedule(e);
			}
			if (((nn = null), (rn = null), this.#h())) {
				(this.#b(r), this.#b(n));
				for (let [e, t] of this.#f) pn(e, t);
				i.length > 0 && N.#g();
				return;
			}
			let o = this.#v();
			if (o) {
				(this.#b(r), this.#b(n), o.#y(this));
				return;
			}
			(this.#u.clear(), this.#d.clear());
			for (let e of this.#r) e(this);
			(this.#r.clear(), (Qt = this), un(r), un(n), (Qt = null), this.#s?.resolve());
			var s = N;
			if (
				(this.#a === 0 && (this.#c.length === 0 || s !== null) && this.#x(), this.#c.length > 0)
			) {
				if (s !== null) {
					let e = s;
					e.#c.push(...this.#c.filter((t) => !e.#c.includes(t)));
				} else s = this;
			}
			s !== null && s.#g();
		}
		#_(e, t, n) {
			e.f ^= C;
			for (var r = e.first; r !== null;) {
				var i = r.f,
					a = !!(i & 96);
				if (!((a && i & 1024) || i & 8192 || this.#f.has(r)) && r.fn !== null) {
					a ? (r.f ^= C) : i & 4 ? t.push(r) : xr(r) && (i & 16 && this.#d.add(r), Er(r));
					var o = r.first;
					if (o !== null) {
						r = o;
						continue;
					}
				}
				for (; r !== null;) {
					var s = r.next;
					if (s !== null) {
						r = s;
						break;
					}
					r = r.parent;
				}
			}
		}
		#v() {
			for (var e = this.#t; e !== null;) {
				if (!e.is_fork) {
					for (let [t, [, n]] of this.current) if (e.current.has(t) && !n) return e;
				}
				e = e.#t;
			}
			return null;
		}
		#y(e) {
			for (let [t, n] of e.current)
				(!this.previous.has(t) && e.previous.has(t) && this.previous.set(t, e.previous.get(t)),
					this.current.set(t, n));
			for (let [t, n] of e.async_deriveds) {
				let e = this.async_deriveds.get(t);
				e && n.promise.then(e.resolve).catch(e.reject);
			}
			(e.async_deriveds.clear(), this.transfer_effects(e.#u, e.#d));
			let t = (e) => {
				var n = e.reactions;
				if (n !== null && !(e.f & 2 && !(e.f & 6144)))
					for (let e of n) {
						var r = e.f;
						if (r & 2) t(e);
						else {
							var i = e;
							r & 4194320 &&
								!this.async_deriveds.has(i) &&
								(this.#d.delete(i), M(i, w), this.schedule(i));
						}
					}
			};
			for (let e of this.current.keys()) t(e);
			(this.oncommit(() => e.discard()), e.#x(), (N = this), this.#g());
		}
		#b(e) {
			for (var t = 0; t < e.length; t += 1) wt(e[t], this.#u, this.#d);
		}
		capture(e, t, n = !1) {
			(e.v !== T && !this.previous.has(e) && this.previous.set(e, e.v),
				e.f & 8388608 || (this.current.set(e, [t, n]), P?.set(e, t)),
				this.is_fork || (e.v = t));
		}
		activate() {
			N = this;
		}
		deactivate() {
			((N = null), (P = null));
		}
		flush() {
			try {
				((tn = !0), (N = this), this.#g());
			} finally {
				((an = 0),
					($t = null),
					(nn = null),
					(rn = null),
					(tn = !1),
					(N = null),
					(P = null),
					_n.clear());
			}
		}
		discard() {
			for (let e of this.#i) e(this);
			this.#i.clear();
			for (let e of this.async_deriveds.values()) e.reject(Ht);
			(this.#x(), this.#s?.resolve());
		}
		register_created_effect(e) {
			this.#l.push(e);
		}
		increment(e, t) {
			if (((this.#a += 1), e)) {
				let e = this.#o.get(t) ?? 0;
				this.#o.set(t, e + 1);
			}
		}
		decrement(e, t) {
			if ((--this.#a, e)) {
				let e = this.#o.get(t) ?? 0;
				e === 1 ? this.#o.delete(t) : this.#o.set(t, e - 1);
			}
			this.#m ||
				((this.#m = !0),
				j(() => {
					((this.#m = !1), this.linked && this.flush());
				}));
		}
		transfer_effects(e, t) {
			for (let t of e) this.#u.add(t);
			for (let e of t) this.#d.add(e);
			(e.clear(), t.clear());
		}
		oncommit(e) {
			this.#r.add(e);
		}
		ondiscard(e) {
			this.#i.add(e);
		}
		settled() {
			return (this.#s ??= te()).promise;
		}
		static ensure() {
			if (N === null) {
				let t = (N = new e());
				!tn &&
					!en &&
					j(() => {
						t.#e || t.flush();
					});
			}
			return N;
		}
		apply() {
			P = null;
		}
		schedule(e) {
			if ((($t = e), e.b?.is_pending && e.f & 16777228 && !(e.f & 32768))) {
				e.b.defer_effect(e);
				return;
			}
			for (var t = e; t.parent !== null;) {
				t = t.parent;
				var n = t.f;
				if (nn !== null && t === G && (H === null || !(H.f & 2))) return;
				if (n & 96) {
					if (!(n & 1024)) return;
					t.f ^= C;
				}
			}
			this.#c.push(t);
		}
		#x() {
			if (this.linked) {
				var e = this.#t,
					t = this.#n;
				(e === null || (e.#n = t), t === null ? (Zt = e) : (t.#t = e), (this.linked = !1));
			}
		}
	};
function cn(e) {
	var t = en;
	en = !0;
	try {
		var n;
		for (e && (N !== null && !N.is_fork && N.flush(), (n = e())); ;) {
			if ((vt(), N === null)) return n;
			N.flush();
		}
	} finally {
		en = t;
	}
}
function ln() {
	try {
		Pe();
	} catch (e) {
		bt(e, $t);
	}
}
var F = null;
function un(e) {
	var t = e.length;
	if (t !== 0) {
		for (var n = 0; n < t;) {
			var r = e[n++];
			if (
				!(r.f & 24576) &&
				xr(r) &&
				((F = new Set()),
				Er(r),
				r.deps === null &&
					r.first === null &&
					r.nodes === null &&
					r.teardown === null &&
					r.ac === null &&
					ir(r),
				F?.size > 0)
			) {
				_n.clear();
				for (let e of F) {
					if (e.f & 24576) continue;
					let t = [e],
						n = e.parent;
					for (; n !== null;) (F.has(n) && (F.delete(n), t.push(n)), (n = n.parent));
					for (let e = t.length - 1; e >= 0; e--) {
						let n = t[e];
						n.f & 24576 || Er(n);
					}
				}
				F.clear();
			}
		}
		F = null;
	}
}
function dn(e, t) {
	if (e.reactions !== null)
		for (let n of e.reactions) {
			let e = n.f;
			e & 2 ? dn(n, t) : e & 131072 && (M(n, w), t.add(n));
		}
}
function fn(e) {
	N.schedule(e);
}
function pn(e, t) {
	if (!(e.f & 32 && e.f & 1024)) {
		(e.f & 2048 ? t.d.push(e) : e.f & 4096 && t.m.push(e), M(e, C));
		for (var n = e.first; n !== null;) (pn(n, t), (n = n.next));
	}
}
function mn(e) {
	M(e, C);
	for (var t = e.first; t !== null;) (mn(t), (t = t.next));
}
function hn(e) {
	(Ee(`fork`), N !== null && Ie());
	var t = sn.ensure();
	((t.is_fork = !0), (P = new Map()));
	var n = !1,
		r = t.settled();
	return (
		cn(e),
		{
			commit: async () => {
				if (n) {
					await r;
					return;
				}
				(t.linked || Fe(), (n = !0), (t.is_fork = !1));
				for (var [e, [i]] of t.current) ((e.v = i), (e.wv = br()));
				(cn(() => {
					var e = new Set();
					for (var n of t.current.keys()) dn(n, e);
					(vn(e), wn());
				}),
					t.flush(),
					await r);
			},
			discard: () => {
				for (var e of t.current.keys()) e.wv = br();
				!n && t.linked && t.discard();
			},
		}
	);
}
var gn = new Set(),
	_n = new Map();
function vn(e) {
	gn = e;
}
var yn = !1;
function bn(e, t) {
	return { f: 0, v: e, reactions: null, equals: nt, rv: 0, wv: 0 };
}
function xn(e, t) {
	let n = bn(e, t);
	return (mr(n), n);
}
function Sn(e, t = !1, n = !0) {
	let r = bn(e);
	return (t || (r.equals = it), r);
}
function I(e, t, n = !1) {
	return (
		H !== null &&
			(!U || H.f & 131072) &&
			pt() &&
			H.f & 4325394 &&
			(q === null || !q.has(e)) &&
			Ue(),
		Cn(e, n ? Dn(t) : t, rn)
	);
}
function Cn(e, t, n = null) {
	if (!e.equals(t)) {
		_n.set(e, fr ? t : e.v);
		var r = sn.ensure();
		if ((r.capture(e, t), e.f & 2)) {
			let t = e;
			(e.f & 2048 && qt(t), P === null && St(t));
		}
		((e.wv = br()),
			En(e, w, n),
			pt() && G !== null && G.f & 1024 && !(G.f & 96) && (X === null ? hr([e]) : X.push(e)),
			!r.is_fork && gn.size > 0 && !yn && wn());
	}
	return t;
}
function wn() {
	yn = !1;
	for (let e of gn) {
		e.f & 1024 && M(e, ne);
		let t;
		try {
			t = xr(e);
		} catch {
			t = !0;
		}
		t && Er(e);
	}
	gn.clear();
}
function Tn(e) {
	I(e, e.v + 1);
}
function En(e, t, n) {
	var r = e.reactions;
	if (r !== null)
		for (var i = pt(), a = r.length, o = 0; o < a; o++) {
			var s = r[o],
				c = s.f;
			if (!(!i && s === G)) {
				var l = (c & w) === 0;
				if ((l && M(s, t), c & 131072)) gn.add(s);
				else if (c & 2) {
					var u = s;
					(P?.delete(u),
						c & 65536 ||
							(c & 512 && (G === null || !(G.f & 2097152)) && (s.f |= fe), En(u, ne, n)));
				} else if (l) {
					var d = s;
					(c & 16 && F !== null && F.add(d), n === null ? fn(d) : n.push(d));
				}
			}
		}
}
function Dn(e) {
	if (typeof e != `object` || !e || ge in e) return e;
	let t = y(e);
	if (t !== _ && t !== v) return e;
	var n = new Map(),
		r = u(e),
		i = xn(0),
		a = null,
		o = vr,
		s = (e) => {
			if (vr === o) return e();
			var t = H,
				n = vr;
			(W(null), yr(o));
			var r = e();
			return (W(t), yr(n), r);
		};
	return (
		r && n.set(`length`, xn(e.length, a)),
		new Proxy(e, {
			defineProperty(e, t, r) {
				(!(`value` in r) || r.configurable === !1 || r.enumerable === !1 || r.writable === !1) &&
					Ve();
				var i = n.get(t);
				return (
					i === void 0
						? s(() => {
								var e = xn(r.value, a);
								return (n.set(t, e), e);
							})
						: I(i, r.value, !0),
					!0
				);
			},
			deleteProperty(e, t) {
				var r = n.get(t);
				if (r === void 0) {
					if (t in e) {
						let e = s(() => xn(T, a));
						(n.set(t, e), Tn(i));
					}
				} else (I(r, T), Tn(i));
				return !0;
			},
			get(t, r, i) {
				if (r === ge) return e;
				var o = n.get(r),
					c = r in t;
				if (
					(o === void 0 &&
						(!c || h(t, r)?.writable) &&
						((o = s(() => xn(Dn(c ? t[r] : T), a))), n.set(r, o)),
					o !== void 0)
				) {
					var l = Z(o);
					return l === T ? void 0 : l;
				}
				return Reflect.get(t, r, i);
			},
			getOwnPropertyDescriptor(e, t) {
				var r = Reflect.getOwnPropertyDescriptor(e, t);
				if (r && `value` in r) {
					var i = n.get(t);
					i && (r.value = Z(i));
				} else if (r === void 0) {
					var a = n.get(t),
						o = a?.v;
					if (a !== void 0 && o !== T)
						return { enumerable: !0, configurable: !0, value: o, writable: !0 };
				}
				return r;
			},
			has(e, t) {
				if (t === ge) return !0;
				var r = n.get(t),
					i = (r !== void 0 && r.v !== T) || Reflect.has(e, t);
				return (r !== void 0 || (G !== null && (!i || h(e, t)?.writable))) &&
					(r === void 0 && ((r = s(() => xn(i ? Dn(e[t]) : T, a))), n.set(t, r)), Z(r) === T)
					? !1
					: i;
			},
			set(e, t, o, c) {
				var l = n.get(t),
					u = t in e;
				if (r && t === `length`)
					for (var d = o; d < l.v; d += 1) {
						var f = n.get(d + ``);
						f === void 0 ? d in e && ((f = s(() => xn(T, a))), n.set(d + ``, f)) : I(f, T);
					}
				if (l === void 0)
					(!u || h(e, t)?.writable) && ((l = s(() => xn(void 0, a))), I(l, Dn(o)), n.set(t, l));
				else {
					u = l.v !== T;
					var p = s(() => Dn(o));
					I(l, p);
				}
				var m = Reflect.getOwnPropertyDescriptor(e, t);
				if ((m?.set && m.set.call(c, o), !u)) {
					if (r && typeof t == `string`) {
						var g = n.get(`length`),
							_ = Number(t);
						Number.isInteger(_) && _ >= g.v && I(g, _ + 1);
					}
					Tn(i);
				}
				return !0;
			},
			ownKeys(e) {
				Z(i);
				var t = Reflect.ownKeys(e).filter((e) => {
					var t = n.get(e);
					return t === void 0 || t.v !== T;
				});
				for (var [r, a] of n) a.v !== T && !(r in e) && t.push(r);
				return t;
			},
			setPrototypeOf() {
				He();
			},
		})
	);
}
var On, kn, An, jn;
function Mn() {
	if (On === void 0) {
		((On = window), (kn = /Firefox/.test(navigator.userAgent)));
		var e = Element.prototype,
			t = Node.prototype,
			n = Text.prototype;
		((An = h(t, `firstChild`).get),
			(jn = h(t, `nextSibling`).get),
			b(e) && ((e[be] = void 0), (e[ye] = null), (e[xe] = void 0), (e.__e = void 0)),
			b(n) && (n[Se] = void 0));
	}
}
function L(e = ``) {
	return document.createTextNode(e);
}
function Nn(e) {
	return An.call(e);
}
function R(e) {
	return jn.call(e);
}
function Pn(e, t) {
	if (!E) return Nn(e);
	var n = Nn(O);
	if (n === null) n = O.appendChild(L());
	else if (t && n.nodeType !== 3) {
		var r = L();
		return (n?.before(r), k(r), r);
	}
	return (t && Bn(n), k(n), n);
}
function Fn(e, t = !1) {
	if (!E) {
		var n = Nn(e);
		return n instanceof Comment && n.data === `` ? R(n) : n;
	}
	if (t) {
		if (O?.nodeType !== 3) {
			var r = L();
			return (O?.before(r), k(r), r);
		}
		Bn(O);
	}
	return O;
}
function In(e, t = 1, n = !1) {
	let r = E ? O : e;
	for (var i; t--;) ((i = r), (r = R(r)));
	if (!E) return r;
	if (n) {
		if (r?.nodeType !== 3) {
			var a = L();
			return (r === null ? i?.after(a) : r.before(a), k(a), a);
		}
		Bn(r);
	}
	return (k(r), r);
}
function Ln(e) {
	e.textContent = ``;
}
function Rn() {
	return !1;
}
function zn(e, t, n) {
	return t == null || t === `http://www.w3.org/1999/xhtml`
		? n
			? document.createElement(e, { is: n })
			: document.createElement(e)
		: n
			? document.createElementNS(t, e, { is: n })
			: document.createElementNS(t, e);
}
function Bn(e) {
	if (e.nodeValue.length < 65536) return;
	let t = e.nextSibling;
	for (; t !== null && t.nodeType === 3;)
		(t.remove(), (e.nodeValue += t.nodeValue), (t = e.nextSibling));
}
function Vn(e) {
	(G === null && (H === null && Ne(e), Me()), fr && je(e));
}
function Hn(e, t) {
	var n = t.last;
	n === null ? (t.last = t.first = e) : ((n.next = e), (e.prev = n), (t.last = e));
}
function z(e, t) {
	var n = G;
	n !== null && n.f & 8192 && (e |= re);
	var r = {
		ctx: A,
		deps: null,
		nodes: null,
		f: e | w | 512,
		first: null,
		fn: t,
		last: null,
		next: null,
		parent: n,
		b: n && n.b,
		prev: null,
		teardown: null,
		wv: 0,
		ac: null,
	};
	N?.register_created_effect(r);
	var i = r;
	if (e & 4) nn === null ? sn.ensure().schedule(r) : nn.push(r);
	else if (t !== null) {
		try {
			Er(r);
		} catch (e) {
			throw (V(r), e);
		}
		i.deps === null &&
			i.teardown === null &&
			i.nodes === null &&
			i.first === i.last &&
			!(i.f & 524288) &&
			((i = i.first), e & 16 && e & 65536 && i !== null && (i.f |= se));
	}
	if (i !== null && ((i.parent = n), n !== null && Hn(i, n), H !== null && H.f & 2 && !(e & 64))) {
		var a = H;
		(a.effects ??= []).push(i);
	}
	return r;
}
function Un() {
	return H !== null && !U;
}
function Wn(e) {
	let t = z(8, null);
	return (M(t, C), (t.teardown = e), t);
}
function Gn(e) {
	Vn(`$effect`);
	var t = G.f;
	if (!H && t & 32 && A !== null && !A.i) {
		var n = A;
		(n.e ??= []).push(e);
	} else return Kn(e);
}
function Kn(e) {
	return z(4 | ue, e);
}
function qn(e) {
	return (Vn(`$effect.pre`), z(8 | ue, e));
}
function Jn(e) {
	sn.ensure();
	let t = z(64 | le, e);
	return (e = {}) =>
		new Promise((n) => {
			e.outro
				? ar(t, () => {
						(V(t), n(void 0));
					})
				: (V(t), n(void 0));
		});
}
function Yn(e) {
	return z(4, e);
}
function Xn(e) {
	return z(me | le, e);
}
function Zn(e, t = 0) {
	return z(8 | t, e);
}
function Qn(e, t = [], n = [], r = []) {
	Lt(r, t, n, (t) => {
		z(8, () => {
			e(...t.map(Z));
		});
	});
}
function $n(e, t = 0) {
	return z(16 | t, e);
}
function B(e) {
	return z(32 | le, e);
}
function er(e) {
	var t = e.teardown;
	if (t !== null) {
		let e = fr,
			n = H;
		(pr(!0), W(null));
		try {
			t.call(null);
		} finally {
			(pr(e), W(n));
		}
	}
}
function tr(e, t = !1) {
	var n = e.first;
	for (e.first = e.last = null; n !== null;) {
		let e = n.ac;
		e !== null &&
			jt(() => {
				e.abort(we);
			});
		var r = n.next;
		(n.f & 64 ? (n.parent = null) : V(n, t), (n = r));
	}
}
function nr(e) {
	for (var t = e.first; t !== null;) {
		var n = t.next;
		(t.f & 32 || V(t), (t = n));
	}
}
function V(e, t = !0) {
	var n = !1;
	((t || e.f & 262144) &&
		e.nodes !== null &&
		e.nodes.end !== null &&
		(rr(e.nodes.start, e.nodes.end), (n = !0)),
		(e.f |= oe),
		tr(e, t && !n),
		Tr(e, 0));
	var r = e.nodes && e.nodes.t;
	if (r !== null) for (let e of r) e.stop();
	(er(e), (e.f ^= oe), (e.f |= ie));
	var i = e.parent;
	(i !== null && i.first !== null && ir(e),
		(e.next = e.prev = e.teardown = e.ctx = e.deps = e.fn = e.nodes = e.ac = e.b = null));
}
function rr(e, t) {
	for (; e !== null;) {
		var n = e === t ? null : R(e);
		(e.remove(), (e = n));
	}
}
function ir(e) {
	var t = e.parent,
		n = e.prev,
		r = e.next;
	(n !== null && (n.next = r),
		r !== null && (r.prev = n),
		t !== null && (t.first === e && (t.first = r), t.last === e && (t.last = n)));
}
function ar(e, t, n = !0) {
	var r = [];
	or(e, r, !0);
	var i = () => {
			(n && V(e), t && t());
		},
		a = r.length;
	if (a > 0) {
		var o = () => --a || i();
		for (var s of r) s.out(o);
	} else i();
}
function or(e, t, n) {
	if (!(e.f & 8192)) {
		e.f ^= re;
		var r = e.nodes && e.nodes.t;
		if (r !== null) for (let e of r) (e.is_global || n) && t.push(e);
		for (var i = e.first; i !== null;) {
			var a = i.next;
			if (!(i.f & 64)) {
				var o = !!(i.f & 65536) || (!!(i.f & 32) && !!(e.f & 16));
				or(i, t, o ? n : !1);
			}
			i = a;
		}
	}
}
function sr(e) {
	cr(e, !0);
}
function cr(e, t) {
	if (e.f & 8192) {
		((e.f ^= re), e.f & 1024 || (M(e, w), sn.ensure().schedule(e)));
		for (var n = e.first; n !== null;) {
			var r = n.next,
				i = !!(n.f & 65536) || !!(n.f & 32);
			(cr(n, i ? t : !1), (n = r));
		}
		var a = e.nodes && e.nodes.t;
		if (a !== null) for (let e of a) (e.is_global || t) && e.in();
	}
}
function lr(e, t) {
	if (e.nodes)
		for (var n = e.nodes.start, r = e.nodes.end; n !== null;) {
			var i = n === r ? null : R(n);
			(t.append(n), (n = i));
		}
}
var ur = null,
	dr = !1,
	fr = !1;
function pr(e) {
	fr = e;
}
var H = null,
	U = !1;
function W(e) {
	H = e;
}
var G = null;
function K(e) {
	G = e;
}
var q = null;
function mr(e) {
	H !== null && (q ??= new Set()).add(e);
}
var J = null,
	Y = 0,
	X = null;
function hr(e) {
	X = e;
}
var gr = 1,
	_r = 0,
	vr = _r;
function yr(e) {
	vr = e;
}
function br() {
	return ++gr;
}
function xr(e) {
	var t = e.f;
	if (t & 2048) return !0;
	if ((t & 2 && (e.f &= ~fe), t & 4096)) {
		for (var n = e.deps, r = n.length, i = 0; i < r; i++) {
			var a = n[i];
			if ((xr(a) && Jt(a), a.wv > e.wv)) return !0;
		}
		t & 512 && P === null && M(e, C);
	}
	return !1;
}
function Sr(e, t, n = !0) {
	var r = e.reactions;
	if (r !== null && !(q !== null && q.has(e)))
		for (var i = 0; i < r.length; i++) {
			var a = r[i];
			a.f & 2 ? Sr(a, t, !1) : t === a && (n ? M(a, w) : a.f & 1024 && M(a, ne), fn(a));
		}
}
function Cr(e) {
	var t = J,
		n = Y,
		r = X,
		i = H,
		a = q,
		o = A,
		s = U,
		c = vr,
		l = e.f;
	((J = null),
		(Y = 0),
		(X = null),
		(H = l & 96 ? null : e),
		(q = null),
		at(e.ctx),
		(U = !1),
		(vr = ++_r),
		e.ac !== null &&
			(jt(() => {
				e.ac.abort(we);
			}),
			(e.ac = null)));
	try {
		e.f |= pe;
		var u = e.fn,
			d = u();
		e.f |= ae;
		var f = e.deps,
			p = N?.is_fork;
		if (J !== null) {
			var m;
			if ((p || Tr(e, Y), f !== null && Y > 0))
				for (f.length = Y + J.length, m = 0; m < J.length; m++) f[Y + m] = J[m];
			else e.deps = f = J;
			if (Un() && e.f & 512) for (m = Y; m < f.length; m++) (f[m].reactions ??= []).push(e);
		} else !p && f !== null && Y < f.length && (Tr(e, Y), (f.length = Y));
		if (pt() && X !== null && !U && f !== null && !(e.f & 6146))
			for (m = 0; m < X.length; m++) Sr(X[m], e);
		if (i !== null && i !== e) {
			if ((_r++, i.deps !== null)) for (let e = 0; e < n; e += 1) i.deps[e].rv = _r;
			if (t !== null) for (let e of t) e.rv = _r;
			X !== null && (r === null ? (r = X) : r.push(...X));
		}
		return (e.f & 8388608 && (e.f ^= he), d);
	} catch (e) {
		return yt(e);
	} finally {
		((e.f ^= pe), (J = t), (Y = n), (X = r), (H = i), (q = a), at(o), (U = s), (vr = c));
	}
}
function wr(e, t) {
	let n = t.reactions;
	if (n !== null) {
		var r = d.call(n, e);
		if (r !== -1) {
			var i = n.length - 1;
			i === 0 ? (n = t.reactions = null) : ((n[r] = n[i]), n.pop());
		}
	}
	if (n === null && t.f & 2 && (J === null || !f.call(J, t))) {
		var a = t;
		(a.f & 512 && ((a.f ^= 512), (a.f &= ~fe)),
			a.v !== T && St(a),
			a.ac !== null &&
				jt(() => {
					(a.ac.abort(we), (a.ac = null), M(a, w));
				}),
			Yt(a),
			Tr(a, 0));
	}
}
function Tr(e, t) {
	var n = e.deps;
	if (n !== null) for (var r = t; r < n.length; r++) wr(e, n[r]);
}
function Er(e) {
	var t = e.f;
	if (!(t & 16384)) {
		M(e, C);
		var n = G,
			r = dr;
		((G = e), (dr = !(t & 96)));
		try {
			(t & 16777232 ? nr(e) : tr(e), er(e));
			var i = Cr(e);
			((e.teardown = typeof i == `function` ? i : null), (e.wv = gr));
		} finally {
			((dr = r), (G = n));
		}
	}
}
async function Dr() {
	(await Promise.resolve(), cn());
}
function Or() {
	return sn.ensure().settled();
}
function Z(e) {
	var t = !!(e.f & 2);
	if ((ur?.add(e), H !== null && !U && !(G !== null && G.f & 16384) && (q === null || !q.has(e)))) {
		var n = H.deps;
		if (H.f & 2097152)
			e.rv < _r &&
				((e.rv = _r),
				J === null && n !== null && n[Y] === e ? Y++ : J === null ? (J = [e]) : J.push(e));
		else {
			((H.deps ??= []), f.call(H.deps, e) || H.deps.push(e));
			var r = e.reactions;
			r === null ? (e.reactions = [H]) : f.call(r, H) || r.push(H);
		}
	}
	if (fr && _n.has(e)) return _n.get(e);
	if (t) {
		var i = e;
		if (fr) {
			var a = i.v;
			return (((!(i.f & 1024) && i.reactions !== null) || Ar(i)) && (a = qt(i)), _n.set(i, a), a);
		}
		var o = !(i.f & 512) && !U && H !== null && (dr || !!(H.f & 512)),
			s = (i.f & ae) === 0;
		(xr(i) && (o && (i.f |= 512), Jt(i)), o && !s && (Xt(i), kr(i)));
	}
	if (P?.has(e)) return P.get(e);
	if (e.f & 8388608) throw e.v;
	return e.v;
}
function kr(e) {
	if (((e.f |= 512), e.deps !== null))
		for (let t of e.deps) ((t.reactions ??= []).push(e), t.f & 2 && !(t.f & 512) && (Xt(t), kr(t)));
}
function Ar(e) {
	if (e.v === T) return !0;
	if (e.deps === null) return !1;
	for (let t of e.deps) if (_n.has(t) || (t.f & 2 && Ar(t))) return !0;
	return !1;
}
function Q(e) {
	var t = U;
	try {
		return ((U = !0), e());
	} finally {
		U = t;
	}
}
[
	...`allowfullscreen.async.autofocus.autoplay.checked.controls.default.disabled.formnovalidate.indeterminate.inert.ismap.loop.multiple.muted.nomodule.novalidate.open.playsinline.readonly.required.reversed.seamless.selected.webkitdirectory.defer.disablepictureinpicture.disableremoteplayback`.split(
		`.`,
	),
];
var jr = [`touchstart`, `touchmove`];
function Mr(e) {
	return jr.includes(e);
}
var Nr = Symbol(`events`),
	Pr = new Set(),
	Fr = new Set();
function Ir(e, t, n, r = {}) {
	function i(e) {
		if ((r.capture || Vr.call(t, e), !e.cancelBubble)) return jt(() => n?.call(this, e));
	}
	return (
		e.startsWith(`pointer`) || e.startsWith(`touch`) || e === `wheel`
			? j(() => {
					t.addEventListener(e, i, r);
				})
			: t.addEventListener(e, i, r),
		i
	);
}
function Lr(e, t, n, r, i) {
	var a = { capture: r, passive: i },
		o = Ir(e, t, n, a);
	(t === document.body || t === window || t === document || t instanceof HTMLMediaElement) &&
		Wn(() => {
			t.removeEventListener(e, o, a);
		});
}
function Rr(e, t, n) {
	(t[Nr] ??= {})[e] = n;
}
function zr(e) {
	for (var t = 0; t < e.length; t++) Pr.add(e[t]);
	for (var n of Fr) n(e);
}
var Br = null;
function Vr(e) {
	var t = this,
		n = t.ownerDocument,
		r = e.type,
		i = e.composedPath?.() || [],
		a = i[0] || e.target;
	Br = e;
	var o = 0,
		s = Br === e && e[Nr];
	if (s) {
		var c = i.indexOf(s);
		if (c !== -1 && (t === document || t === window)) {
			e[Nr] = t;
			return;
		}
		var l = i.indexOf(t);
		if (l === -1) return;
		c <= l && (o = c);
	}
	if (((a = i[o] || e.target), a !== t)) {
		m(e, `currentTarget`, {
			configurable: !0,
			get() {
				return a || n;
			},
		});
		var u = H,
			d = G;
		(W(null), K(null));
		try {
			for (var f, p = []; a !== null && a !== t;) {
				try {
					var h = a[Nr]?.[r];
					h != null && (!a.disabled || e.target === a) && h.call(a, e);
				} catch (e) {
					f ? p.push(e) : (f = e);
				}
				if (e.cancelBubble) break;
				(o++, (a = o < i.length ? i[o] : null));
			}
			if (f) {
				for (let e of p)
					queueMicrotask(() => {
						throw e;
					});
				throw f;
			}
		} finally {
			((e[Nr] = t), delete e.currentTarget, W(u), K(d));
		}
	}
}
var Hr =
	globalThis?.window?.trustedTypes &&
	globalThis.window.trustedTypes.createPolicy(`svelte-trusted-html`, { createHTML: (e) => e });
function Ur(e) {
	return Hr?.createHTML(e) ?? e;
}
function Wr(e) {
	var t = zn(`template`);
	return ((t.innerHTML = Ur(e.replaceAll(`<!>`, `<!---->`))), t.content);
}
function Gr(e, t) {
	var n = G;
	n.nodes === null && (n.nodes = { start: e, end: t, a: null, t: null });
}
function Kr(e, t) {
	var n = !!(t & 1),
		r = !!(t & 2),
		i,
		a = !e.startsWith(`<!>`);
	return () => {
		if (E) return (Gr(O, null), O);
		i === void 0 && ((i = Wr(a ? e : `<!>` + e)), n || (i = Nn(i)));
		var t = r || kn ? document.importNode(i, !0) : i.cloneNode(!0);
		if (n) {
			var o = Nn(t),
				s = t.lastChild;
			Gr(o, s);
		} else Gr(t, t);
		return t;
	};
}
function qr(e = ``) {
	if (!E) {
		var t = L(e + ``);
		return (Gr(t, t), t);
	}
	var n = O;
	return (n.nodeType === 3 ? Bn(n) : (n.before((n = L())), k(n)), Gr(n, n), n);
}
function Jr() {
	if (E) return (Gr(O, null), O);
	var e = document.createDocumentFragment(),
		t = document.createComment(``),
		n = L();
	return (e.append(t, n), Gr(t, n), e);
}
function Yr(e, t) {
	if (E) {
		var n = G;
		((!(n.f & 32768) || n.nodes.end === null) && (n.nodes.end = O), Ze());
		return;
	}
	e !== null && e.before(t);
}
var Xr = !0;
function Zr(e, t) {
	var n = t == null ? `` : typeof t == `object` ? `${t}` : t;
	n !== (e[Se] ??= e.nodeValue) && ((e[Se] = n), (e.nodeValue = `${n}`));
}
function Qr(e, t) {
	return ti(e, t);
}
function $r(e, t) {
	(Mn(), (t.intro = t.intro ?? !1));
	let n = t.target,
		r = E,
		i = O;
	try {
		for (var a = Nn(n); a && (a.nodeType !== 8 || a.data !== `[`);) a = R(a);
		if (!a) throw Ge;
		(D(!0), k(a));
		let r = ti(e, { ...t, anchor: a });
		return (D(!1), r);
	} catch (r) {
		if (
			r instanceof Error &&
			r.message
				.split(
					`
`,
				)
				.some((e) => e.startsWith(`https://svelte.dev/e/`))
		)
			throw r;
		return (
			r !== Ge && console.warn(`Failed to hydrate: `, r),
			t.recover === !1 && Re(),
			Mn(),
			Ln(n),
			D(!1),
			Qr(e, t)
		);
	} finally {
		(D(r), k(i));
	}
}
var ei = new Map();
function ti(
	e,
	{ target: t, anchor: n, props: r = {}, events: i, context: a, intro: o = !0, transformError: s },
) {
	Mn();
	var c = void 0,
		l = Jn(() => {
			var l = n ?? t.appendChild(L());
			Ft(
				l,
				{ pending: () => {} },
				(t) => {
					dt({});
					var n = A;
					if (
						(a && (n.c = a),
						i && (r.$$events = i),
						E && Gr(t, null),
						(Xr = o),
						(c = e(t, r) || {}),
						(Xr = !0),
						E && ((G.nodes.end = O), O === null || O.nodeType !== 8 || O.data !== `]`))
					)
						throw (Ye(), Ge);
					ft();
				},
				s,
			);
			var u = new Set(),
				d = (e) => {
					for (var n = 0; n < e.length; n++) {
						var r = e[n];
						if (!u.has(r)) {
							u.add(r);
							var i = Mr(r);
							for (let e of [t, document]) {
								var a = ei.get(e);
								a === void 0 && ((a = new Map()), ei.set(e, a));
								var o = a.get(r);
								o === void 0
									? (e.addEventListener(r, Vr, { passive: i }), a.set(r, 1))
									: a.set(r, o + 1);
							}
						}
					}
				};
			return (
				d(p(Pr)),
				Fr.add(d),
				() => {
					for (var e of u)
						for (let n of [t, document]) {
							var r = ei.get(n),
								i = r.get(e);
							--i == 0
								? (n.removeEventListener(e, Vr), r.delete(e), r.size === 0 && ei.delete(n))
								: r.set(e, i);
						}
					(Fr.delete(d), l !== n && l.parentNode?.removeChild(l));
				}
			);
		});
	return (ni.set(c, l), c);
}
var ni = new WeakMap();
function ri(e, t) {
	let n = ni.get(e);
	return n ? (ni.delete(e), n(t)) : Promise.resolve();
}
var ii = class {
	anchor;
	#e = new Map();
	#t = new Map();
	#n = new Map();
	#r = new Set();
	#i = !0;
	constructor(e, t = !0) {
		((this.anchor = e), (this.#i = t));
	}
	#a = (e) => {
		if (this.#e.has(e)) {
			var t = this.#e.get(e),
				n = this.#t.get(t);
			if (n) (sr(n), this.#r.delete(t));
			else {
				var r = this.#n.get(t);
				r &&
					(sr(r.effect),
					this.#t.set(t, r.effect),
					this.#n.delete(t),
					r.fragment.lastChild.remove(),
					this.anchor.before(r.fragment),
					(n = r.effect));
			}
			for (let [t, n] of this.#e) {
				if ((this.#e.delete(t), t === e)) break;
				let r = this.#n.get(n);
				r && (V(r.effect), this.#n.delete(n));
			}
			for (let [e, r] of this.#t) {
				if (e === t || this.#r.has(e)) continue;
				let i = () => {
					if (Array.from(this.#e.values()).includes(e)) {
						var t = document.createDocumentFragment();
						(lr(r, t), t.append(L()), this.#n.set(e, { effect: r, fragment: t }));
					} else V(r);
					(this.#r.delete(e), this.#t.delete(e));
				};
				this.#i || !n ? (this.#r.add(e), ar(r, i, !1)) : i();
			}
		}
	};
	#o = (e) => {
		this.#e.delete(e);
		let t = Array.from(this.#e.values());
		for (let [e, n] of this.#n) t.includes(e) || (V(n.effect), this.#n.delete(e));
	};
	ensure(e, t) {
		var n = N,
			r = Rn();
		if (t && !this.#t.has(e) && !this.#n.has(e)) {
			if (r) {
				var i = document.createDocumentFragment(),
					a = L();
				(i.append(a), this.#n.set(e, { effect: B(() => t(a)), fragment: i }));
			} else
				this.#t.set(
					e,
					B(() => t(this.anchor)),
				);
		}
		if ((this.#e.set(n, e), r)) {
			for (let [t, r] of this.#t) t === e ? n.unskip_effect(r) : n.skip_effect(r);
			for (let [t, r] of this.#n) t === e ? n.unskip_effect(r.effect) : n.skip_effect(r.effect);
			(n.oncommit(this.#a), n.ondiscard(this.#o));
		} else (E && (this.anchor = O), this.#a(n));
	}
};
function ai(e, t, n = !1) {
	var r;
	E && ((r = O), Ze());
	var i = new ii(e),
		a = n ? se : 0;
	function o(e, t) {
		if (E) {
			var n = tt(r);
			if (e !== parseInt(n.substring(1))) {
				var a = et();
				(k(a), (i.anchor = a), D(!1), i.ensure(e, t), D(!0));
				return;
			}
		}
		i.ensure(e, t);
	}
	$n(() => {
		var e = !1;
		(t((t, n = 0) => {
			((e = !0), o(n, t));
		}),
			e || o(-1, null));
	}, a);
}
function oi(e, t, n) {
	for (var r = [], i = t.length, a, o = t.length, s = 0; s < i; s++) {
		let n = t[s];
		ar(
			n,
			() => {
				if (a) {
					if ((a.pending.delete(n), a.done.add(n), a.pending.size === 0)) {
						var t = e.outrogroups;
						(si(e, p(a.done)), t.delete(a), t.size === 0 && (e.outrogroups = null));
					}
				} else --o;
			},
			!1,
		);
	}
	if (o === 0) {
		var c = r.length === 0 && n !== null;
		if (c) {
			var l = n,
				u = l.parentNode;
			(Ln(u), u.append(l), e.items.clear());
		}
		si(e, t, !c);
	} else ((a = { pending: new Set(t), done: new Set() }), (e.outrogroups ??= new Set()).add(a));
}
function si(e, t, n = !0) {
	var r;
	if (e.pending.size > 0) {
		r = new Set();
		for (let t of e.pending.values()) for (let n of t) r.add(e.items.get(n).e);
	}
	for (var i = 0; i < t.length; i++) {
		var a = t[i];
		r?.has(a) ? ((a.f |= de), lr(a, document.createDocumentFragment())) : V(t[i], n);
	}
}
var ci;
function li(e, t, n, r, i, a = null) {
	var o = e,
		s = new Map();
	if (t & 4) {
		var c = e;
		o = E ? k(Nn(c)) : c.appendChild(L());
	}
	E && Ze();
	var l = null,
		d = Gt(() => {
			var e = n();
			return u(e) ? e : e == null ? [] : p(e);
		}),
		f,
		m = new Map(),
		h = !0;
	function g(e) {
		v.effect.f & 16384 ||
			(v.pending.delete(e),
			(v.fallback = l),
			di(v, f, o, t, r),
			l !== null &&
				(f.length === 0
					? l.f & 33554432
						? ((l.f ^= de), pi(l, null, o))
						: sr(l)
					: ar(l, () => {
							l = null;
						})));
	}
	function _(e) {
		v.pending.delete(e);
	}
	var v = {
		effect: $n(() => {
			f = Z(d);
			var e = f.length;
			let c = !1;
			E && (tt(o) === `[!`) != (e === 0) && ((o = et()), k(o), D(!1), (c = !0));
			for (var u = new Set(), p = N, v = Rn(), y = 0; y < e; y += 1) {
				E && O.nodeType === 8 && O.data === `]` && ((o = O), (c = !0), D(!1));
				var b = f[y],
					x = r(b, y),
					S = h ? null : s.get(x);
				(S
					? (S.v && Cn(S.v, b), S.i && Cn(S.i, y), v && p.unskip_effect(S.e))
					: ((S = fi(s, h ? o : (ci ??= L()), b, x, y, i, t, n)), h || (S.e.f |= de), s.set(x, S)),
					u.add(x));
			}
			if (
				(e === 0 &&
					a &&
					!l &&
					(h ? (l = B(() => a(o))) : ((l = B(() => a((ci ??= L())))), (l.f |= de))),
				e > u.size && Ae(``, ``, ``),
				E && e > 0 && k(et()),
				!h)
			) {
				if ((m.set(p, u), v)) {
					for (let [e, t] of s) u.has(e) || p.skip_effect(t.e);
					(p.oncommit(g), p.ondiscard(_));
				} else g(p);
			}
			(c && D(!0), Z(d));
		}),
		flags: t,
		items: s,
		pending: m,
		outrogroups: null,
		fallback: l,
	};
	((h = !1), E && (o = O));
}
function ui(e) {
	for (; e !== null && !(e.f & 32);) e = e.next;
	return e;
}
function di(e, t, n, r, i) {
	var a = !!(r & 8),
		o = t.length,
		s = e.items,
		c = ui(e.effect.first),
		l,
		u = null,
		d,
		f = [],
		m = [],
		h,
		g,
		_,
		v;
	if (a)
		for (v = 0; v < o; v += 1)
			((h = t[v]),
				(g = i(h, v)),
				(_ = s.get(g).e),
				_.f & 33554432 || (_.nodes?.a?.measure(), (d ??= new Set()).add(_)));
	for (v = 0; v < o; v += 1) {
		if (((h = t[v]), (g = i(h, v)), (_ = s.get(g).e), e.outrogroups !== null))
			for (let t of e.outrogroups) (t.pending.delete(_), t.done.delete(_));
		if (
			(_.f & 8192 && (sr(_), a && (_.nodes?.a?.unfix(), (d ??= new Set()).delete(_))),
			_.f & 33554432)
		) {
			if (((_.f ^= de), _ === c)) pi(_, null, n);
			else {
				var y = u ? u.next : c;
				(_ === e.effect.last && (e.effect.last = _.prev),
					_.prev && (_.prev.next = _.next),
					_.next && (_.next.prev = _.prev),
					mi(e, u, _),
					mi(e, _, y),
					pi(_, y, n),
					(u = _),
					(f = []),
					(m = []),
					(c = ui(u.next)));
				continue;
			}
		}
		if (_ !== c) {
			if (l !== void 0 && l.has(_)) {
				if (f.length < m.length) {
					var b = m[0],
						x;
					u = b.prev;
					var S = f[0],
						ee = f[f.length - 1];
					for (x = 0; x < f.length; x += 1) pi(f[x], b, n);
					for (x = 0; x < m.length; x += 1) l.delete(m[x]);
					(mi(e, S.prev, ee.next),
						mi(e, u, S),
						mi(e, ee, b),
						(c = b),
						(u = ee),
						--v,
						(f = []),
						(m = []));
				} else
					(l.delete(_),
						pi(_, c, n),
						mi(e, _.prev, _.next),
						mi(e, _, u === null ? e.effect.first : u.next),
						mi(e, u, _),
						(u = _));
				continue;
			}
			for (f = [], m = []; c !== null && c !== _;)
				((l ??= new Set()).add(c), m.push(c), (c = ui(c.next)));
			if (c === null) continue;
		}
		(_.f & 33554432 || f.push(_), (u = _), (c = ui(_.next)));
	}
	if (e.outrogroups !== null) {
		for (let t of e.outrogroups)
			t.pending.size === 0 && (si(e, p(t.done)), e.outrogroups?.delete(t));
		e.outrogroups.size === 0 && (e.outrogroups = null);
	}
	if (c !== null || l !== void 0) {
		var te = [];
		if (l !== void 0) for (_ of l) _.f & 8192 || te.push(_);
		for (; c !== null;) (!(c.f & 8192) && c !== e.fallback && te.push(c), (c = ui(c.next)));
		var C = te.length;
		if (C > 0) {
			var w = r & 4 && o === 0 ? n : null;
			if (a) {
				for (v = 0; v < C; v += 1) te[v].nodes?.a?.measure();
				for (v = 0; v < C; v += 1) te[v].nodes?.a?.fix();
			}
			oi(e, te, w);
		}
	}
	a &&
		j(() => {
			if (d !== void 0) for (_ of d) _.nodes?.a?.apply();
		});
}
function fi(e, t, n, r, i, a, o, s) {
	var c = o & 1 ? (o & 16 ? bn(n) : Sn(n, !1, !1)) : null,
		l = o & 2 ? bn(i) : null;
	return {
		v: c,
		i: l,
		e: B(
			() => (
				a(t, c ?? n, l ?? i, s),
				() => {
					e.delete(r);
				}
			),
		),
	};
}
function pi(e, t, n) {
	if (e.nodes)
		for (
			var r = e.nodes.start, i = e.nodes.end, a = t && !(t.f & 33554432) ? t.nodes.start : n;
			r !== null;
		) {
			var o = R(r);
			if ((a.before(r), r === i)) return;
			r = o;
		}
}
function mi(e, t, n) {
	(t === null ? (e.effect.first = n) : (t.next = n),
		n === null ? (e.effect.last = t) : (n.prev = t));
}
function hi(e, t, ...n) {
	var r = new ii(e);
	$n(() => {
		let e = t() ?? null;
		r.ensure(e, e && ((t) => e(t, ...n)));
	}, se);
}
function gi(e) {
	return (t, ...n) => {
		var r = e(...n),
			i;
		E ? ((i = O), Ze()) : ((i = Nn(Wr(r.render().trim()))), t.before(i));
		let a = r.setup?.(i);
		(Gr(i, i), typeof a == `function` && Wn(a));
	};
}
function _i(e, t, n) {
	var r;
	E && ((r = O), Ze());
	var i = new ii(e);
	$n(() => {
		var e = t() ?? null;
		if (E && (tt(r) === `[`) != (e !== null)) {
			var a = et();
			(k(a), (i.anchor = a), D(!1), i.ensure(e, e && ((t) => n(t, e))), D(!0));
			return;
		}
		i.ensure(e, e && ((t) => n(t, e)));
	}, se);
}
var vi = () => performance.now(),
	$ = { tick: (e) => requestAnimationFrame(e), now: () => vi(), tasks: new Set() };
function yi() {
	let e = $.now();
	($.tasks.forEach((t) => {
		t.c(e) || ($.tasks.delete(t), t.f());
	}),
		$.tasks.size !== 0 && $.tick(yi));
}
function bi(e) {
	let t;
	return (
		$.tasks.size === 0 && $.tick(yi),
		{
			promise: new Promise((n) => {
				$.tasks.add((t = { c: e, f: n }));
			}),
			abort() {
				$.tasks.delete(t);
			},
		}
	);
}
function xi(e, t) {
	jt(() => {
		e.dispatchEvent(new CustomEvent(t));
	});
}
function Si(e) {
	if (e === `float`) return `cssFloat`;
	if (e === `offset`) return `cssOffset`;
	if (e.startsWith(`--`)) return e;
	let t = e.split(`-`);
	return t.length === 1
		? t[0]
		: t[0] +
				t
					.slice(1)
					.map((e) => e[0].toUpperCase() + e.slice(1))
					.join(``);
}
function Ci(e) {
	let t = {},
		n = e.split(`;`);
	for (let e of n) {
		let [n, r] = e.split(`:`);
		if (!n || r === void 0) break;
		let i = Si(n.trim());
		t[i] = r.trim();
	}
	return t;
}
var wi = (e) => e;
function Ti(e, t, n, r) {
	var i = !!(e & 1),
		a = !!(e & 2),
		o = i && a,
		s = !!(e & 4),
		c = o ? `both` : i ? `in` : `out`,
		l,
		u = t.inert,
		d = t.style.overflow,
		f,
		p;
	function m() {
		return jt(() => (l ??= n()(t, r?.() ?? {}, { direction: c })));
	}
	var h = {
			is_global: s,
			in() {
				if (((t.inert = u), !i)) {
					(p?.abort(), p?.reset?.());
					return;
				}
				(a || f?.abort(),
					(f = Ei(
						t,
						m(),
						p,
						1,
						() => {
							xi(t, `introstart`);
						},
						() => {
							(xi(t, `introend`), f?.abort(), (f = l = void 0), (t.style.overflow = d));
						},
					)));
			},
			out(e) {
				if (!a) {
					(e?.(), (l = void 0));
					return;
				}
				((t.inert = !0),
					(p = Ei(
						t,
						m(),
						f,
						0,
						() => {
							xi(t, `outrostart`);
						},
						() => {
							(xi(t, `outroend`), e?.());
						},
					)));
			},
			stop: () => {
				(f?.abort(), p?.abort());
			},
		},
		g = G;
	if (((g.nodes.t ??= []).push(h), i && Xr)) {
		var _ = s;
		if (!_) {
			for (var v = g.parent; v && v.f & 65536;) for (; (v = v.parent) && !(v.f & 16););
			_ = !v || !!(v.f & 32768);
		}
		_ &&
			Yn(() => {
				Q(() => h.in());
			});
	}
}
function Ei(e, t, n, r, i, a) {
	var o = r === 1;
	if (x(t)) {
		var s,
			c = !1;
		return (
			j(() => {
				c || (s = Ei(e, t({ direction: o ? `in` : `out` }), n, r, i, a));
			}),
			{
				abort: () => {
					((c = !0), s?.abort());
				},
				deactivate: () => s.deactivate(),
				reset: () => s.reset(),
				t: () => s.t(),
			}
		);
	}
	if ((n?.deactivate(), !t?.duration && !t?.delay))
		return (i(), a(), { abort: S, deactivate: S, reset: S, t: () => r });
	let { delay: l = 0, css: u, tick: d, easing: f = wi } = t;
	var p = [];
	if (o && n === void 0 && (d && d(0, 1), u)) {
		var m = Ci(u(0, 1));
		p.push(m, m);
	}
	var h = () => 1 - r,
		g = e.animate(p, { duration: l, fill: `forwards` });
	return (
		(g.onfinish = () => {
			(g.cancel(), i());
			var o = n?.t() ?? 1 - r;
			n?.abort();
			var s = r - o,
				c = t.duration * Math.abs(s),
				l = [];
			if (c > 0) {
				var p = !1;
				if (u)
					for (var m = Math.ceil(c / (1e3 / 60)), _ = 0; _ <= m; _ += 1) {
						var v = o + s * f(_ / m),
							y = Ci(u(v, 1 - v));
						(l.push(y), (p ||= y.overflow === `hidden`));
					}
				(p && (e.style.overflow = `hidden`),
					(h = () => {
						var e = g.currentTime;
						return o + s * f(e / c);
					}),
					d &&
						bi(() => {
							if (g.playState !== `running`) return !1;
							var e = h();
							return (d(e, 1 - e), !0);
						}));
			}
			((g = e.animate(l, { duration: c, fill: `forwards` })),
				(g.onfinish = () => {
					((h = () => r), d?.(r, 1 - r), a());
				}));
		}),
		{
			abort: () => {
				g && (g.cancel(), (g.effect = null), (g.onfinish = S));
			},
			deactivate: () => {
				a = S;
			},
			reset: () => {
				r === 0 && d?.(1, 0);
			},
			t: () => h(),
		}
	);
}
function Di(e, t) {
	let n = null,
		r = E;
	var i;
	if (E) {
		n = O;
		for (var a = Nn(document.head); a !== null && (a.nodeType !== 8 || a.data !== e);) a = R(a);
		if (a === null) D(!1);
		else {
			var o = R(a);
			(a.remove(), k(o));
		}
	}
	E || (i = document.head.appendChild(L()));
	try {
		$n(() => {
			var e = B(() => t(i));
			e.f |= ce;
		});
	} finally {
		r && (D(!0), k(n));
	}
}
var Oi = [
	...` 	
\r\f\xA0\v﻿`,
];
function ki(e, t, n) {
	var r = e == null ? `` : `` + e;
	if ((t && (r = r ? r + ` ` + t : t), n)) {
		for (var i of Object.keys(n))
			if (n[i]) r = r ? r + ` ` + i : i;
			else if (r.length)
				for (var a = i.length, o = 0; (o = r.indexOf(i, o)) >= 0;) {
					var s = o + a;
					(o === 0 || Oi.includes(r[o - 1])) && (s === r.length || Oi.includes(r[s]))
						? (r = (o === 0 ? `` : r.substring(0, o)) + r.substring(s + 1))
						: (o = s);
				}
	}
	return r === `` ? null : r;
}
function Ai(e, t = !1) {
	var n = t ? ` !important;` : `;`,
		r = ``;
	for (var i of Object.keys(e)) {
		var a = e[i];
		a != null && a !== `` && (r += ` ` + i + `: ` + a + n);
	}
	return r;
}
function ji(e) {
	return e[0] !== `-` || e[1] !== `-` ? e.toLowerCase() : e;
}
function Mi(e, t) {
	if (t) {
		var n = ``,
			r,
			i;
		if ((Array.isArray(t) ? ((r = t[0]), (i = t[1])) : (r = t), e)) {
			e = String(e)
				.replaceAll(/\s*\/\*.*?\*\/\s*/g, ``)
				.trim();
			var a = !1,
				o = 0,
				s = !1,
				c = [];
			(r && c.push(...Object.keys(r).map(ji)), i && c.push(...Object.keys(i).map(ji)));
			var l = 0,
				u = -1;
			let t = e.length;
			for (var d = 0; d < t; d++) {
				var f = e[d];
				if (
					(s
						? f === `/` && e[d - 1] === `*` && (s = !1)
						: a
							? a === f && (a = !1)
							: f === `/` && e[d + 1] === `*`
								? (s = !0)
								: f === `"` || f === `'`
									? (a = f)
									: f === `(`
										? o++
										: f === `)` && o--,
					!s && a === !1 && o === 0)
				) {
					if (f === `:` && u === -1) u = d;
					else if (f === `;` || d === t - 1) {
						if (u !== -1) {
							var p = ji(e.substring(l, u).trim());
							if (!c.includes(p)) {
								f !== `;` && d++;
								var m = e.substring(l, d).trim();
								n += ` ` + m + `;`;
							}
						}
						((l = d + 1), (u = -1));
					}
				}
			}
		}
		return (r && (n += Ai(r)), i && (n += Ai(i, !0)), (n = n.trim()), n === `` ? null : n);
	}
	return e == null ? null : String(e);
}
function Ni(e, t, n, r, i, a) {
	var o = e[be];
	if (E || o !== n || o === void 0) {
		var s = ki(n, r, a);
		((!E || s !== e.getAttribute(`class`)) &&
			(s == null ? e.removeAttribute(`class`) : t ? (e.className = s) : e.setAttribute(`class`, s)),
			(e[be] = n));
	} else if (a && i !== a)
		for (var c in a) {
			var l = !!a[c];
			(i == null || l !== !!i[c]) && e.classList.toggle(c, l);
		}
	return a;
}
function Pi(e, t = {}, n, r) {
	for (var i in n) {
		var a = n[i];
		t[i] !== a && (n[i] == null ? e.style.removeProperty(i) : e.style.setProperty(i, a, r));
	}
}
function Fi(e, t, n, r) {
	var i = e[xe];
	if (E || i !== t) {
		var a = Mi(t, r);
		((!E || a !== e.getAttribute(`style`)) &&
			(a == null ? e.removeAttribute(`style`) : (e.style.cssText = a)),
			(e[xe] = t));
	} else
		r && (Array.isArray(r) ? (Pi(e, n?.[0], r[0]), Pi(e, n?.[1], r[1], `important`)) : Pi(e, n, r));
	return r;
}
var Ii = Symbol(`is custom element`),
	Li = Symbol(`is html`),
	Ri = Te ? `link` : `LINK`;
function zi(e) {
	if (E) {
		var t = !1,
			n = () => {
				if (!t) {
					if (((t = !0), e.hasAttribute(`value`))) {
						var n = e.value;
						(Bi(e, `value`, null), (e.value = n));
					}
					if (e.hasAttribute(`checked`)) {
						var r = e.checked;
						(Bi(e, `checked`, null), (e.checked = r));
					}
				}
			};
		((e[Ce] = n), j(n), At());
	}
}
function Bi(e, t, n, r) {
	var i = Vi(e);
	(E &&
		((i[t] = e.getAttribute(t)),
		t === `src` || t === `srcset` || (t === `href` && e.nodeName === Ri))) ||
		(i[t] !== (i[t] = n) &&
			(t === `loading` && (e[ve] = n),
			n == null
				? e.removeAttribute(t)
				: typeof n != `string` && Ui(e).includes(t)
					? (e[t] = n)
					: e.setAttribute(t, n)));
}
function Vi(e) {
	return (e[ye] ??= { [Ii]: e.nodeName.includes(`-`), [Li]: e.namespaceURI === Ke });
}
var Hi = new Map();
function Ui(e) {
	var t = e.getAttribute(`is`) || e.nodeName,
		n = Hi.get(t);
	if (n) return n;
	Hi.set(t, (n = []));
	for (var r, i = e, a = Element.prototype; a !== i;) {
		for (var o in ((r = g(i)), r))
			r[o].set && o !== `innerHTML` && o !== `textContent` && o !== `innerText` && n.push(o);
		i = y(i);
	}
	return n;
}
function Wi(e, t, n = t) {
	var r = new WeakSet();
	(Mt(e, `input`, async (i) => {
		var a = i ? e.defaultValue : e.value;
		if (((a = Gi(e) ? Ki(a) : a), n(a), N !== null && r.add(N), await Dr(), a !== (a = t()))) {
			var o = e.selectionStart,
				s = e.selectionEnd,
				c = e.value.length;
			if (((e.value = a ?? ``), s !== null)) {
				var l = e.value.length;
				o === s && s === c && l > c
					? ((e.selectionStart = l), (e.selectionEnd = l))
					: ((e.selectionStart = o), (e.selectionEnd = Math.min(s, l)));
			}
		}
	}),
		((E && e.defaultValue !== e.value) || (Q(t) == null && e.value)) &&
			(n(Gi(e) ? Ki(e.value) : e.value), N !== null && r.add(N)),
		Zn(() => {
			var n = t();
			if (e === document.activeElement) {
				var i = N;
				if (r.has(i)) return;
			}
			(Gi(e) && n === Ki(e.value)) ||
				(e.type === `date` && !n && !e.value) ||
				(n !== e.value && (e.value = n ?? ``));
		}));
}
function Gi(e) {
	var t = e.type;
	return t === `number` || t === `range`;
}
function Ki(e) {
	return e === `` ? null : +e;
}
function qi(e, t) {
	return e === t || e?.[ge] === t;
}
function Ji(e = {}, t, n, r) {
	var i = A.r,
		a = G;
	return (
		Yn(() => {
			var o, s;
			return (
				Zn(() => {
					((o = s),
						(s = r?.() || []),
						Q(() => {
							qi(n(...s), e) || (t(e, ...s), o && qi(n(...o), e) && t(null, ...o));
						}));
				}),
				() => {
					let r = a;
					for (; r !== i && r.parent !== null && r.parent.f & 33554432;) r = r.parent;
					let o = () => {
							s && qi(n(...s), e) && t(null, ...s);
						},
						c = r.teardown;
					r.teardown = () => {
						(o(), c?.());
					};
				}
			);
		}),
		e
	);
}
function Yi(e, t, n, r) {
	var i = !0,
		a = !!(n & 8),
		o = !!(n & 16),
		s = r,
		c = !0,
		l = void 0,
		u = () => (o && i ? ((l ??= Vt(r)), Z(l)) : (c && ((c = !1), (s = o ? Q(r) : r)), s));
	let d;
	if (a) {
		var f = ge in e || _e in e;
		d = h(e, t)?.set ?? (f && t in e ? (n) => (e[t] = n) : void 0);
	}
	var p,
		m = !1;
	(a ? ([p, m] = Ot(() => e[t])) : (p = e[t]),
		p === void 0 && r !== void 0 && ((p = u()), d && (i && Be(t), d(p))));
	var g = i
		? () => {
				var n = e[t];
				return n === void 0 ? u() : ((c = !0), n);
			}
		: () => {
				var n = e[t];
				return (n !== void 0 && (s = void 0), n === void 0 ? s : n);
			};
	if (i && !(n & 4)) return g;
	if (d) {
		var _ = e.$$legacy;
		return function (e, t) {
			return arguments.length > 0 ? ((!i || !t || _ || m) && d(t ? g() : e), e) : g();
		};
	}
	var v = !1,
		y = (n & 1 ? Vt : Gt)(() => ((v = !1), g()));
	a && Z(y);
	var b = G;
	return function (e, t) {
		if (arguments.length > 0) {
			let n = t ? Z(y) : i && a ? Dn(e) : e;
			return (I(y, n), (v = !0), s !== void 0 && (s = n), e);
		}
		return (fr && v) || b.f & 16384 ? y.v : Z(y);
	};
}
function Xi(e) {
	return class extends Zi {
		constructor(t) {
			super({ component: e, ...t });
		}
	};
}
var Zi = class {
	#e;
	#t;
	constructor(e) {
		var t = new Map(),
			n = (e, n) => {
				var r = Sn(n, !1, !1);
				return (t.set(e, r), r);
			};
		let r = new Proxy(
			{ ...(e.props || {}), $$events: {} },
			{
				get(e, r) {
					return Z(t.get(r) ?? n(r, Reflect.get(e, r)));
				},
				has(e, r) {
					return r === _e || (Z(t.get(r) ?? n(r, Reflect.get(e, r))), Reflect.has(e, r));
				},
				set(e, r, i) {
					return (I(t.get(r) ?? n(r, i), i), Reflect.set(e, r, i));
				},
			},
		);
		((this.#t = (e.hydrate ? $r : Qr)(e.component, {
			target: e.target,
			anchor: e.anchor,
			props: r,
			context: e.context,
			intro: e.intro ?? !1,
			recover: e.recover,
			transformError: e.transformError,
		})),
			(!e?.props?.$$host || e.sync === !1) && cn(),
			(this.#e = r.$$events));
		for (let e of Object.keys(this.#t))
			e !== `$set` &&
				e !== `$destroy` &&
				e !== `$on` &&
				m(this, e, {
					get() {
						return this.#t[e];
					},
					set(t) {
						this.#t[e] = t;
					},
					enumerable: !0,
				});
		((this.#t.$set = (e) => {
			Object.assign(r, e);
		}),
			(this.#t.$destroy = () => {
				ri(this.#t);
			}));
	}
	$set(e) {
		this.#t.$set(e);
	}
	$on(e, t) {
		this.#e[e] = this.#e[e] || [];
		let n = (...e) => t.call(this, ...e);
		return (
			this.#e[e].push(n),
			() => {
				this.#e[e] = this.#e[e].filter((e) => e !== n);
			}
		);
	}
	$destroy() {
		this.#t.$destroy();
	}
};
function Qi(e, t) {
	if ((Ee(`hydratable`), E)) {
		let t = window.__svelte?.h;
		if (t?.has(e)) return t.get(e);
		Je(e);
	}
	return t();
}
var $i = s({
	afterUpdate: () => oa,
	beforeUpdate: () => aa,
	createContext: () => ot,
	createEventDispatcher: () => ia,
	createRawSnippet: () => gi,
	flushSync: () => cn,
	fork: () => hn,
	getAbortSignal: () => ea,
	getAllContexts: () => ut,
	getContext: () => st,
	hasContext: () => lt,
	hydratable: () => Qi,
	hydrate: () => $r,
	mount: () => Qr,
	onDestroy: () => na,
	onMount: () => ta,
	setContext: () => ct,
	settled: () => Or,
	tick: () => Dr,
	unmount: () => ri,
	untrack: () => Q,
});
function ea() {
	return (H === null && Le(), (H.ac ??= new AbortController()).signal);
}
function ta(e) {
	(A === null && De(`onMount`),
		Gn(() => {
			let t = Q(e);
			if (typeof t == `function`) return t;
		}));
}
function na(e) {
	(A === null && De(`onDestroy`), ta(() => () => Q(e)));
}
function ra(e, t, { bubbles: n = !1, cancelable: r = !1 } = {}) {
	return new CustomEvent(e, { detail: t, bubbles: n, cancelable: r });
}
function ia() {
	let e = A;
	return (
		e === null && De(`createEventDispatcher`),
		(t, n, r) => {
			let i = e.s.$$events?.[t];
			if (i) {
				let a = u(i) ? i.slice() : [i],
					o = ra(t, n, r);
				for (let t of a) t.call(e.x, o);
				return !o.defaultPrevented;
			}
			return !0;
		}
	);
}
function aa(e) {
	(A === null && De(`beforeUpdate`), A.l === null && ze(`beforeUpdate`), sa(A).b.push(e));
}
function oa(e) {
	(A === null && De(`afterUpdate`), A.l === null && ze(`afterUpdate`), sa(A).a.push(e));
}
function sa(e) {
	var t = e.l;
	return (t.u ??= { a: [], b: [], m: [] });
}
export {
	Gn as A,
	Et as B,
	Rr as C,
	Dr as D,
	Or as E,
	In as F,
	Qe as G,
	ft as H,
	Dn as I,
	o as K,
	I as L,
	On as M,
	Pn as N,
	Q as O,
	Fn as P,
	xn as R,
	zr as S,
	Z as T,
	dt as U,
	st as V,
	ct as W,
	Zr as _,
	Ji as a,
	Kr as b,
	Bi as c,
	Di as d,
	Ti as f,
	ai as g,
	li as h,
	Yi as i,
	qn as j,
	Qn as k,
	Fi as l,
	hi as m,
	ta as n,
	Wi as o,
	_i as p,
	l as q,
	Xi as r,
	zi as s,
	$i as t,
	Ni as u,
	Yr as v,
	Lr as w,
	qr as x,
	Jr as y,
	Wt as z,
};
