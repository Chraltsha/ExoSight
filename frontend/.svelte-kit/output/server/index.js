		...
		try {
			...
			rendered = await with_request_store(
				...
				async () => {
					...
					const rendered =
						options.async && 'then' in maybe_promise ? maybe_promise.then((r) => r) : maybe_promise;
			...
			promises.map((p, i) =>
				p.catch(async (error) => {
					...
					return {
						...
						error: await handle_error_and_jsonify(event, event_state, options, error),
						status:
							error instanceof HttpError || error instanceof SvelteKitError ? error.status : void 0,
					};
				}),
			),
		);
